...
static void ModulesPerNonTCPUDP(struct ip *pip, void *plast, u_long fpnum);
...
Bool docheck_hw_order = FALSE;
Bool strip_hw_dups = FALSE;
...
static struct ext_bool_op {
    ...
    {"showect", &show_ect,  TRUE,
     "show ECT(0) and ECT(1) explicitly"},
    ...
    {"check_hwdups", &docheck_hw_dups, TRUE,
     "check for IP-level dups"},
    {"check_hwdord", &docheck_hw_order, TRUE,
     "check for IP-level reordering"},
    {"strip_hwdups", &strip_hw_dups, TRUE,
     "ignore 'hardware' dups"},
    ...
    {"rpc", &rpc, TRUE,
     "display decoded RPC segment groups"},
    {"vxlan", &do_vxlan, TRUE,
     "decode vxlan-embedded packets"},
    {"filenames_shark", &filenames_shark, TRUE,
     "use wire/tshark-connecstion-numbered names for plotfiles"},
...
static struct ext_var_op {
    ...
    {"rscale", &rscale, NULL,
     "rtt display scale (m=msec, u=usec)"},
    {"tcpw", &tcpw, NULL,
     "window scale binary exponent default (e.g. 7). Prefix with ~ for auto"},
    {"tscale", &tscale, NULL,
...
Graphing options\n\
  ...
  -Q      create XID time sequence graph[s]\n\
...
Graphing detail options\n\
  ...
  -AN     Average N segments for throughput/rtt graphs, default is 10\n\
  -z      zero axis options\n\
...
embedded:
	...
	if (ret < 0) {
	    /* look for a dest-unreach ICMP (often frag-needed) */
	    if (geticmp(pip, &picmp, &plast) == 0 && picmp->icmp_type == 3) {
		icmphdr_destunreach * pif = (icmphdr_destunreach *) picmp;
		is_icmp = TRUE;
		pip = (struct ip *) &pif->icmp_iphdr;
		ret = gettcp (pip, &ptcp, &plast);
	    }
	}
	if (ret < 0) {
		...
		/* neither UDP not TCP */
		ModulesPerNonTCPUDP(pip ,plast, fpnum);
	    }

	    if (do_vxlan && ret == 0 && ntohs(pudp->dest) == 4789) {
		char * wrapped = (char *)(pudp+1);
		pip = (struct ip *) (wrapped + 8 + 14);
		goto embedded;
	    }
	    continue;
	}
	...
	ptp = dotrace(pip,ptcp,plast, fpnum);
    ...
    char *argv[])
{
	...
	if (*argv[i] == '-') {
		...
		switch (*argv[i]) {
		  ...
		  case 'Q': graph_xsg = graph_tsg = TRUE; break;
...
static void
ModulesPerNonTCPUDP(struct ip *pip, void *plast, u_long fpnum)
{
    int i;
	...
	(*modules[i].module_nontcpudp_read)(pip, plast, fpnum);
    }
}
