    ...
    void **pplast)
{
	...
	/* except when ip_len is zero, which we see for TCP segmentation offload; in which
	   case we continue to trust the capture length */
	if (  pip->ip_len != 0
	   && ((char *)pip + ntohs(pip->ip_len) - 1) < (char *)(*pplast)
	   )
...
gettcp(
    ...
    void **pplast)
{
    return findheader(IPPROTO_TCP, pip, (void **)pptcp, pplast);
}
...
getudp(
    ...
    void **pplast)
{
   return findheader(IPPROTO_UDP, pip, (void **)ppudp, pplast);
}



/*
 * getsctp:  return a pointer to a udp header.
 * Skips either ip or ipv6 headers
 */
int
getsctp(
    ...
    void **pplast)
{
   return findheader(IPPROTO_SCTP, pip, (void **)ppsctp, pplast);
}



/*
 * geticmp:  return a pointer to a icmp header.
 * Skips ip headers
 */
int
geticmp(
    struct ip *pip,
    struct icmphdr **ppicmp,
    void **pplast)
{
   return PIP_ISV4(pip)
	? findheader(IPPROTO_ICMP, pip, (void **)ppicmp, pplast)
	: -1;
}
...
int gethdrlength (struct ip *pip, void *plast)
{
	...
	fprintf(stderr, "excessive number of IPv6 headers\n");
	return -1;
    }
    else
    {
...
int getpayloadlength (struct ip *pip, void *plast)
{
    ...
    if (!(len = ntohs(pip->ip_len)))
	len = plast - (void *)pip;	/* When TSO, trust the capture length */
    len -= IP_HL(pip) * 4;
    return len;
}
