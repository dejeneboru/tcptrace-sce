...
void
DoThru(tcb *ptcb, int nbytes, int totbytes)
{
    double etime;
    double thruput;
    char *myname, *hisname;
	...
	snprintf(title,sizeof(title),"%s_==>_%s (throughput)",
		myname, hisname);
	snprintf(ylabel,sizeof(ylabel),"thruput (%.1sbytes/sec)",
		tscale ? tscale : "");
	...
	if (tscale) switch (*tscale) {
	case 'k': thruput /= 1000; break;
	case 'm': case 'M': thruput /= 1000*1000; break;
	case 'g': case 'G': thruput /= 1000*1000*1000; break;
	}
	...
	if (etime == 0.0)
	    etime = 1000;	/* ick, what if "no time" has passed?? */
	thruput = totbytes ? (double) totbytes : ptcb->thru_seq;
	thruput /= ((double) etime / 1000000.0);
	if (tscale) switch (*tscale) {
	case 'k': thruput /= 1000; break;
	case 'M': thruput /= 1000*1000; break;
	case 'G': thruput /= 1000*1000*1000; break;
	}
