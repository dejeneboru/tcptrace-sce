...
static Bool check_hw_reorder(u_short id, tcb *ptcb);
    ...
    struct tcphdr *ptcp)
{
    ...
    if (filenames_shark) {
      ptp->a2b.host_letter = "a";
      ptp->b2a.host_letter = "b";
    } else {
    ...
    /* init XID time sequence graphs */
    ptp->a2b.xsg_plotter = ptp->b2a.xsg_plotter = NO_PLOTTER;
    if (graph_xsg && !ptp->ignore_pair) {
	if (!ignore_non_comp || (SYN_SET(ptcp))) {
	    snprintf(title,sizeof(title),"%s_==>_%s (XID time sequence graph)",
		    ptp->a_endpoint, ptp->b_endpoint);
	    ptp->a2b.xsg_plotter =
		new_plotter(&ptp->a2b,NULL,title,
			    graph_time_zero?"relative time":"time",
			    graph_seq_zero?"XID offset":"XID number",
			    XSG_FILE_EXTENSION);
	    snprintf(title,sizeof(title),"%s_==>_%s (XID time sequence graph)",
		    ptp->b_endpoint, ptp->a_endpoint);
	    ptp->b2a.xsg_plotter =
	    ...
	    if (graph_time_zero) {
		/* set graph zero points */
		plotter_nothing(ptp->a2b.xsg_plotter, current_time);
		plotter_nothing(ptp->b2a.xsg_plotter, current_time);
	    }
	    ptp->a2b.ack_xid_time.tv_sec = ptp->b2a.ack_xid_time.tv_sec = -1;
	}
    }
    ...
    ptp->a2b.rpc_size = (u_int) -1;
    ptp->b2a.rpc_size = (u_int) -1;
...
static void
frame_info(PLOTTER pl, ulong fpnum, struct ip *pip)
{
    plotter_info(pl, "frame: %u    IPid 0x%04hx", (u_int)fpnum, htons(pip->ip_id));
}
    ...
    u_long fpnum)
{
    ...
    /* check for ignoring hardware duplicates */
    ...
    if (RESET_SET(ptcp)) {
	++thisdir->reset_count;
	if (!thisdir->fin_count && !otherdir->fin_count) ++thisdir->rst_before_fin;
    }
    if (FIN_SET(ptcp))
	++thisdir->fin_count;
	...
	/* DIDN'T see the SYNs, the windows will be off. */
 	...
 	if (!SYN_SET(ptcp))
		...
		else if ((!thisdir->syn_count || ptrunc))
		    {
		    if (!thisdir->tcpw && !thisdir->tcpw_auto)
			if (*tcpw == '~')
			    {
			    thisdir->tcpw_auto = TRUE;
			    thisdir->tcpw = atoi(tcpw+1);
			    }
			else
			    thisdir->tcpw = atoi(tcpw);
		    eff_win <<= thisdir->tcpw;
		    }
    } else
	eff_win = 0;
...
%s->%s: rexmitted FIN had diff. seqnum! (was %lu, now %lu, etime: %d sec)\n",
    ...
    /* check the options */
    ptcpo = ParseOptions(ptcp, plast, &ptrunc);
    if (ptcpo->mss != -1)
	thisdir->mss = ptcpo->mss;
    if (ptcpo->ws != -1 && !ptrunc) {
	thisdir->window_scale = ptcpo->ws;
	thisdir->f1323_ws = TRUE;
    } else if (SYN_SET(ptcp) && ptrunc && tcpw) {
	/* truncated options, but we have a cmdline wscale.  Use that. */
	if (*tcpw == '~')
	    {
	    thisdir->tcpw_auto = TRUE;
	    thisdir->tcpw = atoi(tcpw+1);
    ...
    if (1) {
	if (ptp_save->packets == 1) {
	    plotter_variable(from_tsgpl, "f", ptp_save->filename);
	    plotter_variable(from_tsgpl, "c", "shell wireshark -g");
	    plotter_variable(to_tsgpl,   "f", ptp_save->filename);
	    plotter_variable(to_tsgpl,   "c", "shell wireshark -g");
	}

	/*XXX should this text be configurable? */
	snprintf(clicky_wireshark, sizeof(clicky_wireshark),
	    "$c %d $f", fpnum);
    } else {
	*clicky_wireshark = 0;
    }
    ...
    if (graph_tput && tcp_data_length > 0) switch (*twhich) {
    case 'c': DoThru(thisdir, tcp_data_length, thisdir->data_bytes);
	      break;
    case 't': if (  thisdir->min_seq != 0
    		 && SEQ_GREATERTHAN (end, thisdir->max_seq))
		DoThru(thisdir,
			(int)((end - thisdir->max_seq) & 0xffffffffUL),
			0);
	      break;
    }
    ...
    /* only works for IPv4, IPv6 has no mandatory ID field */
    if (PIP_ISV4(pip) && !is_icmp && docheck_hw_dups) {
	if (!strip_hw_dups)
	    hw_dup = check_hw_dups(pip->ip_id, th_seq, thisdir);
	if (docheck_hw_order)
	    hw_reorder = check_hw_reorder(ntohs(pip->ip_id), thisdir);
    }
    ...
    if (PIP_ISV4(pip)) {
	/* only works for IPv4 */
	ect0 = IP_ECT(pip) && !IP_CE(pip);
	ect1 = !IP_ECT(pip) && IP_CE(pip);
	ecn_ce = IP_ECT(pip) && IP_CE(pip);
    } else if (PIP_ISV6(pip)) {
	struct ipv6 * pipv6 = (void *)pip;
	int tc  = (ntohl(pipv6->ip6_ver_tc_flabel) & 0x0FF00000) >> 20;
	ect0 = tc & IPTOS_ECT && !(tc & IPTOS_CE);
    ...
    /* plot ICMP frag-needed packets */
    if (is_icmp) {
	if (from_tsgpl != NO_PLOTTER) {
	    extern icmphdr * picmp;
	    static char * dus[] = {"DNU","DHU","DPU","DpU","FN","SRF","DNU","DHU",
		    "SHI","NAP","HAP","NUT","HUT","CAP","HPV","PC"};
	    char * s = picmp->icmp_code <= 15 ? dus[picmp->icmp_code] : NULL;
	    char buf[64];

	    if (!s) { sprintf(buf, "icmp 3/%u", (unsigned) picmp->icmp_code); s = buf; }
    ...
    if (from_tsgpl != NO_PLOTTER && show_out_order && (hw_reorder || out_order)) {
	char * letters = hw_reorder ? "HO" : "O";
	plotter_perm_color(from_tsgpl, out_order_color);
	plotter_text(from_tsgpl, current_time, SeqRep(thisdir,end),
		     hw_reorder ? "al":"a", letters);
	if (bottom_letters)
	...
	if (SYN_SET(ptcp)) {		/* SYN  */
	    ...
	    plotter_text(from_tsgpl, current_time,
			 SeqRep(thisdir,start+1+tcp_data_length), "a",
			 hw_dup?"HD SYN":
			 retrans_num_bytes>0?"R SYN":
			 "SYN");
	    if(tcp_data_length == 0)   /* SYN only */
		plotter_uarrow(from_tsgpl, current_time,
			SeqRep(thisdir,start+1+tcp_data_length));
	    plotter_line(from_tsgpl,
			 current_time, SeqRep(thisdir,start),
			 current_time, SeqRep(thisdir,start+1));
	    if (*clicky_wireshark)
		plotter_clickarea(from_tsgpl, clicky_wireshark);
	    if(tcp_data_length > 0) { /* SYN + DATA */
		plotter_uarrow(from_tsgpl, current_time,
			SeqRep(thisdir,start+1+tcp_data_length));
		plotter_perm_color(from_tsgpl,
			       hw_dup?hw_dup_color:
		...
		plotter_line(from_tsgpl,
			 current_time, SeqRep(thisdir,start+1),
			 current_time, SeqRep(thisdir,start+1+tcp_data_length));
	    }
	    frame_info(from_tsgpl, fpnum, pip);
	    ...
	    */
	    ...
	    int captured_len = (char *)plast - ((char *)ptcp + TH_OFF(ptcp)*4) + 1;

	    plotter_perm_color(from_tsgpl,
		hw_dup ? hw_dup_color : retrans ? retrans_color : data_color);
	    ...
	    /* Mark limited-frame-size captured segments */
	    if (captured_len < tcp_data_length)
		plotter_ltick(from_tsgpl, current_time,
		    SeqRep(thisdir,start + captured_len));
	    ...
	    if (rpc) {					/* RPC tracking */
		seqnum seg_start = SeqRep(thisdir,start);
		seqnum seg_end =   SeqRep(thisdir,end);
		struct rpchdr_min {
		    u_int32_t	len;
		    u_int32_t	xid;
		    u_int32_t	type;
		} * rpchdr_min = (struct rpchdr_min *)
		    ((char *)ptcp+tcp_length-tcp_data_length);
		struct rpchdr {
		    struct rpchdr_min min;
		    u_int32_t	ver;
		    u_int32_t	prog;
		    u_int32_t	progver;
		    u_int32_t	proc;
...
nextrpc:
		rpchdr = (struct rpchdr *) rpchdr_min;
		if (  thisdir->rpc_size == (u_int)-1	/* search for RPC start */
		   ...
		   && (ntohl(rpchdr_min->type) != 0 || ntohl(rpchdr->ver) == 2)) {
		    ...
		    thisdir->rpc_size = (ntohl(rpchdr_min->len) & ~(1<<31)) + 4;
		    thisdir->rpc_start_time = current_time;
		    thisdir->rpc_seq_lo = seg_start;
		    thisdir->rpc_seq_hi = seg_start + thisdir->rpc_size;
		    thisdir->rpc_type = ntohl(rpchdr_min->type);

		    if (  thisdir->rpc_type == 0		/* commands */
		       && seg_end-seg_start >= sizeof(*rpchdr)) {
			thisdir->rpc_prog =    ntohl(rpchdr->prog);
			thisdir->rpc_progver = ntohl(rpchdr->progver);
			thisdir->rpc_proc =    ntohl(rpchdr->proc);
		    }
		...
		if (  thisdir->rpc_size != (u_int) -1
		   && (  seg_end == thisdir->rpc_seq_hi
		      || SEQ_GREATERTHAN(seg_end, thisdir->rpc_seq_hi))) {
		    /* We are tracking an RPC, and it ended by the end of
		    this segment */
		    ...
		    if (seg_start == SeqRep(thisdir,start))
			plotter_perm_color(from_tsgpl, "pink");

		    if (SEQ_GREATERTHAN(seg_start, thisdir->rpc_seq_lo)) {
			if (SEQ_GREATERTHAN(thisdir->rpc_seq_hi, seg_start)) {
			    /* The RPC started in a previous seg and ended in this
			    specific segment; draw box from start to here */

			    if (seg_start == SeqRep(thisdir,start))
				frame_info(from_tsgpl, fpnum, pip);

			    plotter_line(from_tsgpl,
				     thisdir->rpc_start_time, thisdir->rpc_seq_lo,
				     thisdir->rpc_start_time, thisdir->rpc_seq_hi);
			    plotter_line(from_tsgpl,
				     thisdir->rpc_start_time, thisdir->rpc_seq_hi,
				     current_time,            thisdir->rpc_seq_hi);
			    plotter_line(from_tsgpl,
				     current_time,            thisdir->rpc_seq_hi,
				     current_time,            thisdir->rpc_seq_lo);
			    plotter_line(from_tsgpl,
				     current_time,            thisdir->rpc_seq_lo,
				     thisdir->rpc_start_time, thisdir->rpc_seq_lo);
			    plotter_info(from_tsgpl, "XID: %u", thisdir->rpc_xid);

			    plotter_box(from_tsgpl,
				     current_time, thisdir->rpc_seq_hi);
			} else {
			    /* The RPC started in a previous seg and ended in some
			    missing seg before this one; draw just the LH end of
			    the box */
			    plotter_rtick(from_tsgpl, thisdir->rpc_start_time,
				thisdir->rpc_seq_lo);
			    plotter_line(from_tsgpl,
				     thisdir->rpc_start_time, thisdir->rpc_seq_lo,
				     thisdir->rpc_start_time, thisdir->rpc_seq_hi);
			...
			/* RPC fully enclosed in this segment */
			plotter_line(from_tsgpl,
				     thisdir->rpc_start_time, thisdir->rpc_seq_lo,
				     thisdir->rpc_start_time, thisdir->rpc_seq_hi);
			    plotter_info(from_tsgpl, "XID: %u", thisdir->rpc_xid);
		    ...
		    plotter_box(from_tsgpl,
				 thisdir->rpc_start_time, thisdir->rpc_seq_lo);
		    plotter_text(from_tsgpl, thisdir->rpc_start_time,
				 thisdir->rpc_seq_hi, "br",
				 thisdir->rpc_type == 0 ? "c"
				 : thisdir->rpc_type == 1 ? "r" : "?");

		    if (thisdir->rpc_type && thisdir->rpc_xid == otherdir->rpc_xid)
			graph_rpc_rtt_sample(otherdir, elapsed(otherdir->rpc_start_time, current_time));
			...
			rpchdr_min = (struct rpchdr_min *)
			    (((char *)ptcp+tcp_length) - (seg_end - seg_start));
			goto nextrpc;
		    }
		} else
		    frame_info(from_tsgpl, fpnum, pip);
	...
	if (!show_ect) {
		...
		plotter_text(from_tsgpl, current_time, SeqRep(thisdir, start), "b", "ECN");
		if (!ACK_SET(ptcp) && !cwr) {
		  plotter_temp_color(from_tsgpl, retrans_color);
		  plotter_text(from_tsgpl, current_time, SeqRep(thisdir, start), "br", " !CWR");
		}
	      }
	    } else
	    /* Kevin Lahey's code */
   ...
   if (tlinepl != NO_PLOTTER) {
	...
	plot_at = ACK_SET(ptcp) ? th_ack : thisdir->ack;

	if (to_tsgpl != NO_PLOTTER) {
	    plotter_temp_color(to_tsgpl, text_color);
	    plotter_text(to_tsgpl, current_time, SeqRep(otherdir,plot_at),
			 "a", hw_dup ? "HD RST_IN" : "RST_IN");
	    if (!ACK_SET(ptcp)) {
		plotter_dtick(to_tsgpl, current_time, SeqRep(otherdir,plot_at));
		if (*clicky_wireshark)
	...
	if (from_tsgpl != NO_PLOTTER) {
	    ...
	    plotter_text(from_tsgpl, current_time, SeqRep(thisdir,start),
			 "a", hw_dup ? "HD RST_OUT" : "RST_OUT");
	}
	if (ACK_SET(ptcp))
	    ++thisdir->ack_pkts;
	...
	if (to_tsgpl != NO_PLOTTER && thisdir->time.tv_sec == -1) {
	    ...
	    if (thisdir->f1323_ws && otherdir->f1323_ws)
		snprintf(str, sizeof(str), "WS %d", thisdir->window_scale);
	    else if (thisdir->syn_count && !ptrunc)
		s = "no WS";
	    else if (tcpw)
		snprintf(str, sizeof(str), "tcpw %s", tcpw);
	    else
		s = "WS est 0";
	    plotter_text(to_tsgpl, current_time,
			 SeqRep(otherdir,winend), "l", s);
	}
	if (to_tsgpl != NO_PLOTTER && thisdir->time.tv_sec != -1) {
		...
		if (eff_win != 0 && thisdir->tcpw_auto)
			...
			if (thisdir->tcpw_hold)
			    ...
			    {			/* window edge absolute seqnum decreased */
			    plotter_temp_color(to_tsgpl, retrans_color);
			    if (thisdir->tcpw > 0)
				{				/* decrease our assessment of wscale */
				char buf[16];
	...
	if (to_tsgpl != NO_PLOTTER && show_sacks
		...
		*/
		...
		if (SEQ_GREATERTHAN(sb->sack_right, sack_top))
		    sack_top = sb->sack_right;
  ...
  if (!run_continuously) {
	...
	for (ix = 0; ix <= num_tcp_pairs; ++ix) {
		...
		if ((printbrief) && (!ignore_non_comp || ConnComplete(ptp))) {
		    fprintf(stdout,"%3d: ", filenames_shark ? ix : ix+1);
		    PrintBrief(ptp);
...
struct tcp_options *
ParseOptions(struct tcphdr *ptcp, void *plast, Bool * ptrunc)
{
...
static Bool
check_hw_reorder(u_short id, tcb *tcb)
{
    Bool ret = FALSE;
    int diff = (id - tcb->hw_inorder_id) & 0x7fff;
    if (diff >= SEGS_DISORDER_DISTANCE) {
	if (  tcb->hw_inorder_count > 0
	   && --tcb->hw_inorder_count >= SEGS_INORDER_OK)
	    ret = TRUE;
    } else {
	if (tcb->hw_inorder_count < SEGS_INORDER_OK*2)
	    tcb->hw_inorder_count++;
    }
    tcb->hw_inorder_id = id;
    return ret;
}
