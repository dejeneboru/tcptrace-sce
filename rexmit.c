		 ...
		 double etime_rtt)
{
    ...
    if (!rscale || *rscale != 'u') etime_rtt /= 1000; /* convert from us to ms */
...
/* graph RTT samples in milliseconds or microseconds */
static void
		  ...
		  unsigned long etime_rtt)
{
	...
	snprintf (title, sizeof (title), "%s_==>_%s (rtt samples)",
		  name_from, name_to);
	ptcb->rtt_plotter =
	    new_plotter (ptcb, NULL, title, "time",
		         !rscale || *rscale != 'u' ? "rtt (ms)" : "rtt (us)",
	...
	ptcb->rtt_avg_line = new_line (ptcb->rtt_plotter, "srtt", "blue");
	ptcb->rtt_avg = (!rscale || *rscale != 'u') ? etime_rtt / 1000 : etime_rtt;

	ptcb->rpc_rtt_line = new_line (ptcb->rtt_plotter, "rpc_rtt", "pink");
	ptcb->rpc_rtt_avg_line = new_line (ptcb->rtt_plotter, "rpc_srtt", "blue");
	ptcb->rpc_rtt_avg = 0;
    }
    ...
    if (!rscale || *rscale != 'u') etime_rtt /= 1000;
    extend_line (ptcb->rtt_line, current_time, (int) etime_rtt);

    ptcb->rtt_avg = (ptcb->rtt_avg * (thru_interval-1) + etime_rtt) / thru_interval;
    extend_line (ptcb->rtt_avg_line, current_time, (int) ptcb->rtt_avg);
}


void
graph_rpc_rtt_sample(tcb * ptcb, unsigned long etime_rtt)
{
    if (  !graph_rtt
       || ptcb->rtt_plotter == NO_PLOTTER
       || etime_rtt <= 1)
	return;

    if (!rscale || *rscale != 'u') etime_rtt /= 1000;
    if (ptcb->rpc_rtt_avg == 0)
	ptcb->rpc_rtt_avg = etime_rtt;
    else {
	extend_line (ptcb->rpc_rtt_line, current_time, (int) etime_rtt);

	ptcb->rpc_rtt_avg = (ptcb->rpc_rtt_avg * (thru_interval-1) + etime_rtt) / thru_interval;
	extend_line (ptcb->rpc_rtt_avg_line, current_time, (int) ptcb->rpc_rtt_avg);
    }
}
