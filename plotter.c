...
static char *TSGPlotName(tcb *plast, char *suffix);
...
char *
PlotName(int num, char * c1, char * c2, char * suffix)
{
    static char filename[25]; 

    if (filenames_shark)
      snprintf(filename, sizeof(filename), "%d_%s2%s%s", num, c1, c2, suffix);
    else
      snprintf(filename, sizeof(filename), "%s2%s%s", c1, c2, suffix);
    return(filename);
}


static char *
TSGPlotName(tcb *plast, char *suffix)
{
    return PlotName(plast->ptp->pair_number, plast->host_letter, plast->ptwin->host_letter, suffix);
}
    ...
    char *suffix)
{
    ...
    if (filename == NULL)
	filename = TSGPlotName(plast, suffix);
    else if (suffix != NULL) {
	char buf[100];
...
plotter_perm_color(
    ...
    char *color)
{
    static PLOTTER last_pl = -1;
    static char * lastcolor = NULL;

   if (colorplot && (pl != last_pl || color != lastcolor))
	{
...
plotter_clickarea(
    ...
    char *text)
{
    CallDoPlot(pl,"clickarea", 0);
    CallDoPlot(pl,text, 0);
}


void
plotter_info(PLOTTER pl, const char *text, ...)
{
    va_list ap;
    char buf[128];
    va_start(ap, text);
...
plotter_box2(
    ...
    u_long		x2)
{
    CallDoPlot(pl,"box2", 4, t1, x1, t2, x2);
}
    ...
    PLOTTER pl)
{
   ...
   if(ppi->axis_switched) {   
      ...
      Mfprintf(f,"%s %s\n",
	       ...
	       || strncmp(ppi->ylabel, "thruput", 7) == 0
	       || strncmp(ppi->ylabel, "XID number", 10) == 0
	       ? "double":"signed");
   }
...
/* Switch the x and y axis type (Needed for Time Line Charts. Default = FALSE) */
