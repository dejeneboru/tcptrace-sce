...
ConnComplete(
    tcp_pair *ptp)
{
    return(SynCount(ptp) >= 2 && ((FinCount(ptp) >= 2) || ptp->a2b.reset_count > 0 || ptp->b2a.reset_count > 0));
}
...
ConnReset(
    tcp_pair *ptp)
{
    return(ptp->a2b.rst_before_fin + ptp->b2a.rst_before_fin != 0);
}
    ...
    tcp_pair *ptp)
{
    ...
    if (filenames_shark)
      snprintf(infobuf,sizeof(infobuf),"%s - %s",
    ...
    tcp_pair *ptp)
{
	...
	*/ 
    ...
    if (pab->rst_before_fin || pba->rst_before_fin)
	StatLineI("resets sent before fin","", pab->rst_before_fin, pba->rst_before_fin);
    ...
    pab_last = (pab->rst_before_fin>0)?pab->latest_seq:pab->fin;
    
    pba_last = (pba->rst_before_fin>0)?pba->latest_seq:pba->fin;
    ...
    ");
	    ...
	    StatLineF("RTT min","us","%8.1f",
		      (double)pab->rtt_min,
		      (double)pba->rtt_min);
	    StatLineF("RTT max","us","%8.1f",
		      (double)pab->rtt_max,
		      (double)pba->rtt_max);
	    StatLineF("RTT avg","us","%8.1f",
		      Average(pab->rtt_sum, pab->rtt_count),
		      Average(pba->rtt_sum, pba->rtt_count));
	    StatLineF("RTT stdev","us","%8.1f",
		      Stdev(pab->rtt_sum, pab->rtt_sum2, pab->rtt_count),
		      Stdev(pba->rtt_sum, pba->rtt_sum2, pba->rtt_count));
	    if(!(csv || tsv || (sv != NULL)))
	      fprintf(stdout,"\n");
	    StatLineF("RTT from 3WHS","us","%8.1f",
		      (double)pab->rtt_3WHS,
		      (double)pba->rtt_3WHS);
	    if(!(csv || tsv || (sv != NULL)))
	      fprintf(stdout,"\n");
	    StatLineI("RTT full_sz smpls","", 
		      pab->rtt_full_count, pba->rtt_full_count);
	    StatLineF("RTT full_sz min","us","%8.1f",
		      (double)pab->rtt_full_min,
		      (double)pba->rtt_full_min);
	    StatLineF("RTT full_sz max","us","%8.1f",
		      (double)pab->rtt_full_max,
		      (double)pba->rtt_full_max);
	    StatLineF("RTT full_sz avg","us","%8.1f",
		      Average(pab->rtt_full_sum, pab->rtt_full_count),
		      Average(pba->rtt_full_sum, pba->rtt_full_count));
	    StatLineF("RTT full_sz stdev","us","%8.1f",
		      Stdev(pab->rtt_full_sum, pab->rtt_full_sum2, pab->rtt_full_count),
		      Stdev(pba->rtt_full_sum, pba->rtt_full_sum2, pba->rtt_full_count));
	    if(!(csv || tsv || (sv != NULL)))
	      fprintf(stdout,"\n");
	    StatLineI("post-loss acks","",
		      pab->rtt_nosample, pba->rtt_nosample);
    ...
    ");
		StatLineI("ambiguous acks","",
			  pab->rtt_amback, pba->rtt_amback);
		StatLineF("RTT min (last)","us","%8.1f",
			  (double)pab->rtt_min_last,
			  (double)pba->rtt_min_last);
		StatLineF("RTT max (last)","us","%8.1f",
			  (double)pab->rtt_max_last,
			  (double)pba->rtt_max_last);
		StatLineF("RTT avg (last)","us","%8.1f",
			  Average(pab->rtt_sum_last, pab->rtt_count_last),
			  Average(pba->rtt_sum_last, pba->rtt_count_last));
		StatLineF("RTT sdv (last)","us","%8.1f",
			  Stdev(pab->rtt_sum_last, pab->rtt_sum2_last, pab->rtt_count_last),
			  Stdev(pba->rtt_sum_last, pba->rtt_sum2_last, pba->rtt_count_last));
	    ...
	    StatLineI("max # retrans","",
		      pab->retr_max, pba->retr_max);
	    StatLineF("min retr time","us","%8.1f",
		      (double)((double)pab->retr_min_tm),
		      (double)((double)pba->retr_min_tm));
	    StatLineF("max retr time","us","%8.1f",
		      (double)((double)pab->retr_max_tm),
		      (double)((double)pba->retr_max_tm));
	    StatLineF("avg retr time","us","%8.1f",
		      Average(pab->retr_tm_sum, pab->retr_tm_count),
		      Average(pba->retr_tm_sum, pba->retr_tm_count));
	    StatLineF("sdv retr time","us","%8.1f",
		      Stdev(pab->retr_tm_sum, pab->retr_tm_sum2,
			    pab->retr_tm_count),
		      Stdev(pba->retr_tm_sum, pba->retr_tm_sum2,
			    pba->retr_tm_count));
    ...
    tcp_pair *ptp)
{
    ...
    if (ConnReset(ptp))
        fprintf(stdout,
	      ptp->a2b.rst_before_fin == 0 ? "  (reset<)"
	    : ptp->b2a.rst_before_fin == 0 ? "  (reset>)"
	    : "  (reset<>)"
	    );
