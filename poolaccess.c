...
static long sctp_pair_pool = -1;
static long udp_pair_pool = -1;
static long ip_pair_pool = -1;
	    ...
	    void)
{
  ...
  if (tcp_pair_pool < 0) tcp_pair_pool = MakeMemPool(sizeof(tcp_pair), 0);
...
MakeSctpPair(void)
{
  ...
  if (sctp_pair_pool < 0) sctp_pair_pool = MakeMemPool(sizeof(sctp_pair), 0);
  ptr = PoolMalloc(sctp_pair_pool, sizeof(sctp_pair));
  return ptr;
}

void
FreeSctpPair(sctp_pair *ptr)
{
  PoolFree(sctp_pair_pool, ptr);
}
#endif
...
MakeIpPair(void)
{
  ...
  if (ip_pair_pool < 0) ip_pair_pool = MakeMemPool(sizeof(ip_pair), 0);
  ptr = PoolMalloc(ip_pair_pool, sizeof(ip_pair));
  return ptr;
}

void
FreeIpPair(ip_pair *ptr)
{
  PoolFree(ip_pair_pool, ptr);
}
#endif
	    ...
	    void)
{
  ...
  if (udp_pair_pool < 0) udp_pair_pool = MakeMemPool(sizeof(udp_pair), 0);
	     ...
	     void)
{
  ...
  if (seqspace_pool < 0) seqspace_pool = MakeMemPool(sizeof(seqspace), 0);
	    ...
	    void)
{
  ...
  if (ptp_snap_pool < 0) ptp_snap_pool = MakeMemPool(sizeof(ptp_snap), 0);
	   ...
	   void)
{
  ...
  if (ptp_ptr_pool < 0) ptp_ptr_pool = MakeMemPool(sizeof(ptp_ptr), 0);
...
MakeSegment(void)
{
  ...
  if (segment_pool < 0) segment_pool = MakeMemPool(sizeof(segment), 0);
...
MakeQuadrant(void)
{
  ...
  if (quadrant_pool < 0) quadrant_pool = MakeMemPool(sizeof(quadrant), 0);
