...
typedef struct tcb {
    ...
    u_char	rst_before_fin;  /* resets SENT, before a FIN (either way) is seen */
    u_long	min_seg_size;
    u_long	max_seg_size;
...
#define SEGS_TO_REMEMBER 16
    ...
    /* hardware reordering detection */
    u_short	hw_inorder_id;	/* IP ID */
    u_char	hw_inorder_count;
				/* If no more than this jump fwd,      */
				/* plausible incrementally-counting IP.*/
#define SEGS_INORDER_OK		8
				/* If jumps fwd more than this, disordered.*/
				/* must be large due to multiplexing of    */
				/* traffic onto the IP layer. Most         */
				/* disorder will be spotted on jump back.  */
#define SEGS_DISORDER_DISTANCE	16384
    ...
    double	thru_seq;	/* number of bytes this connection */
    ...
    u_char	tcpw;		/* initialiased from cmdline */
    u_char	tcpw_hold;	/* auto-eval tcpw holdoff */
    Bool	tcpw_auto;	/* auto-eval tcpw */

    /* RPC tracking */
    struct timeval rpc_start_time;
    seqnum	rpc_seq_lo;
    seqnum	rpc_seq_hi;
    u_int	rpc_xid;
...
typedef struct icmphdr {
    ...
    u_int8_t	icmp_code;
    u_int16_t	icmp_csum;
    int		icmp_data[1];	/* variable size */
} icmphdr;

typedef struct icmphdr_destunreach {
    u_int8_t	icmp_type;
    u_int8_t	icmp_code;
    u_int16_t	icmp_csum;
    u_int16_t	icmp_unused;
    u_int16_t	icmp_nxt_mtu;
    struct iphdr icmp_iphdr;	/* variable size */
} icmphdr_destunreach;
...
extern Bool do_udp;
extern Bool do_vxlan;
extern Bool show_title;
extern Bool show_rwinline;
extern Bool docheck_hw_dups;
extern Bool docheck_hw_order;
extern Bool strip_hw_dups;
extern Bool rpc;
extern Bool filenames_shark;
...
void plotter_variable(PLOTTER pl, char *name, char *value);
void plotter_clickarea(PLOTTER pl, char *text);
void plotter_info(PLOTTER pl, const char *text, ...);
...
void plotter_box2(PLOTTER, timeval, u_long, timeval, u_long);
...
void plot_init(void);
tcp_pair *dotrace(struct ip *, struct tcphdr *ptcp, void *plast, u_long fpnum);
...
char *PlotName(int num, char * c1, char * c2, char * suffix);
...
struct tcp_options *ParseOptions(struct tcphdr *ptcp, void *plast, Bool * ptrunc);
...
#define NS_SET(ptcp)		(TH_X2((ptcp))    & TH_NS)
...
#define TH_NS  0x01		/* RFC3540 ECN Nonce (historic) */
...
#define TSN_FILE_EXTENSION		"_ssg.xpl"
#define XSG_FILE_EXTENSION		"_xsg.xpl"
